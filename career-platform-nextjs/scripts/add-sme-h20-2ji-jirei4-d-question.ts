/**
 * 中小企業診断士 平成20年度 第2次試験 事例Ⅳ（D社・粉体塗装）を certification_questions に追加（または更新）
 *
 * 実行（career-platform-nextjs で）:
 *   npx ts-node --transpile-only scripts/add-sme-h20-2ji-jirei4-d-question.ts
 */
import fs from 'fs';
...
const QUESTION_ID = 'sme-h20-2ji-jirei4-d';
...
【第1問】予想BS・PL・製造原価報告書から、収益性（例：売上高営業利益率・ROA・総資本利益率）、安全性（自己資本比率・流動比率・固定長期適合率）、費用構造（修繕費・減価償却の同業比）などから問題点を3つ選び、指標名・数値・60字以内の説明を答案用紙形式で。

【第2問】現設備継続の税引前営業CF（売上−ランニング、償却は非資金のため調整）、処分時点の処分価額、5年間を10％で割り引く。年金係数3.7908を問題の指示どおり使用。

【第3問】売却損・新設備減価償却・借入利息・節税効果の現在価値。実効税率40％。係数は問題文の値をそのまま用いる。

【第4問】全額借入の財務制約・レバレッジ、Z社出資と経営権（種類株・議決権制限付株式・持分比率設計など）を字数内で。

※BSの細目はPDFと照合してください。OCRで欠けた数値は公式問題集で補完してください。`;
...
async function main() {
  ...
  const txtPath = path.join(__dirname, 'data', 'sme-h20-2ji-jirei4-d-question.txt');
