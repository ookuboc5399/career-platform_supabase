/**
 * 中小企業診断士 平成19年度 第1次試験「財務・会計」（2日目）第1～17問を
 * certification_questions に1レコードで追加（または更新）
 *
 * 実行（career-platform-nextjs で）:
 *   npx ts-node --transpile-only scripts/add-sme-h19-1ji-finance-question.ts
 */
import fs from 'fs';
...
const QUESTION_ID = 'sme-h19-1ji-finance';
...
const EXPLANATION = `【本問について】第1次試験・財務・会計（選択式）の問題文を1件にまとめています。第1問の表、第2問の精算表、第10問の限界利益率・稼働時間、第12問の％、第14問の利子率、第15問の株数、第16問設問2のリスクプレミアム、第17問の利子率などはOCR欠落箇所があります。図表は省略または要旨のみです。

【学習のすすめ】会計基準・会社法・金融商品は改正されています。正答と数値は公式問題集・解答解説で確認してください。
...
async function main() {
  ...
  const p1 = fs.readFileSync(path.join(__dirname, 'data', 'sme-h19-1ji-finance-part1.txt'), 'utf8');
  const p2 = fs.readFileSync(path.join(__dirname, 'data', 'sme-h19-1ji-finance-part2.txt'), 'utf8');
  const questionText =
    p1 +
    '\n' +
    p2 +
  ...
  const row: Record<string, unknown> = {
    ...
    choices: [
      {
        id: 'a',
        text: '【1次・選択式】上記は第1～17問（設問含む）の問題文です。表・数値の欠落は公式問題集で補完し、正答は解答解説書で確認してください。',
      },
      {
        id: 'b',
