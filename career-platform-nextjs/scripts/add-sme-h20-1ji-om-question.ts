/**
 * 中小企業診断士 平成20年度 第1次試験「運営管理（オペレーション・マネジメント）」（2日目）全42問を
 * certification_questions に1レコードで追加（または更新）
 *
 * 実行（career-platform-nextjs で）:
 *   npx ts-node --transpile-only scripts/add-sme-h20-1ji-om-question.ts
 */
import fs from 'fs';
...
const QUESTION_ID = 'sme-h20-1ji-om';
...
const EXPLANATION = `【本問について】第1次試験・運営管理（オペレーション・マネジメント）（選択式・全42問）の問題文を1件にまとめています。第8問の解答群数値、第16問の速度・図、第18問の処理時間表など、OCR欠落箇所は公式問題集で補完してください。

【学習のすすめ】生産管理・小売・物流・法規は試験年度時点の用語と現行の差を意識してください。
...
async function main() {
  ...
  const p1 = fs.readFileSync(path.join(__dirname, 'data', 'sme-h20-1ji-om-part1.txt'), 'utf8');
  const p2 = fs.readFileSync(path.join(__dirname, 'data', 'sme-h20-1ji-om-part2.txt'), 'utf8');
  const questionText =
    p1 +
    '\n' +
    p2 +
  ...
  const row: Record<string, unknown> = {
    ...
    year: 'H20年1次',
    category: '運営管理（オペレーション・マネジメント）',
    choices: [
      {
        id: 'a',
        text: '【1次・選択式】上記は全42問の問題文です。図表欠落は公式問題集で補完し、正答は解答解説書で確認してください。',
      },
      {
        id: 'b',
