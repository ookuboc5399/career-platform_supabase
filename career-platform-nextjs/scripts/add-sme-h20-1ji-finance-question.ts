/**
 * 中小企業診断士 平成20年度 第1次試験「財務・会計」（1日目）第1～25問を
 * certification_questions に1レコードで追加（または更新）
 *
 * 実行（career-platform-nextjs で）:
 *   npx ts-node --transpile-only scripts/add-sme-h20-1ji-finance-question.ts
 */
import fs from 'fs';
...
const QUESTION_ID = 'sme-h20-1ji-finance';
...
const EXPLANATION = `【本問について】第1次試験・財務・会計（選択式・全25問）の問題文を1件にまとめています。第4問の公債条件（利率・経過利息日数）、第15問の社債の符号表記、第19問の投資利益率の表、第20問の図などは公式問題集PDFと照合してください。

【学習のすすめ】会計基準・金融商品取引法は改正されています。正答は解答解説書で確認してください。
...
async function main() {
  ...
  const p1 = fs.readFileSync(path.join(__dirname, 'data', 'sme-h20-1ji-finance-part1.txt'), 'utf8');
  const p2 = fs.readFileSync(path.join(__dirname, 'data', 'sme-h20-1ji-finance-part2.txt'), 'utf8');
  const questionText =
    p1 +
    '\n' +
    p2 +
  ...
  const row: Record<string, unknown> = {
    ...
    choices: [
      {
        id: 'a',
        text: '【1次・選択式】上記は第1～25問の問題文です。表・数式の欠落は公式問題集で補完し、正答は解答解説書で確認してください。',
      },
      {
        id: 'b',
