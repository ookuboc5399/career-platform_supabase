/**
 * 中小企業診断士 平成19年度 第1次試験「運営管理（オペレーション・マネジメント）」（2日目）全41問を
 * certification_questions に1レコードで追加（または更新）
 *
 * 実行（career-platform-nextjs で）:
 *   npx ts-node --transpile-only scripts/add-sme-h19-1ji-om-question.ts
 */
import fs from 'fs';
...
const QUESTION_ID = 'sme-h19-1ji-om';
...
const EXPLANATION = `【本問について】第1次試験・運営管理（オペレーション・マネジメント）（選択式・全41問）の問題文を1件にまとめています。第6問の〇個数、第9・17問の図、第14問の表、第21問設問2・3の選択肢などはOCR欠落のため要確認です。
...
async function main() {
  ...
  const p1 = fs.readFileSync(path.join(__dirname, 'data', 'sme-h19-1ji-om-part1.txt'), 'utf8');
  const p2 = fs.readFileSync(path.join(__dirname, 'data', 'sme-h19-1ji-om-part2.txt'), 'utf8');
  const questionText =
    p1 +
    '\n' +
    p2 +
  ...
  const row: Record<string, unknown> = {
    ...
    choices: [
      {
        id: 'a',
        text: '【1次・選択式】上記は全41問の問題文です。図表欠落は公式問題集で補完し、正答は解答解説書で確認してください。',
      },
      {
        id: 'b',
