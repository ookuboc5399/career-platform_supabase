/**
 * 中小企業診断士 平成19年度 第1次試験「経営情報システム」（2日目）第1～24問を
 * certification_questions に1レコードで追加（または更新）
 *
 * 実行（career-platform-nextjs で）:
 *   npx ts-node --transpile-only scripts/add-sme-h19-1ji-mis-question.ts
 */
import fs from 'fs';
...
const QUESTION_ID = 'sme-h19-1ji-mis';
...
const EXPLANATION = `【本問について】第1次試験・経営情報システム（選択式・第1～24問収録）の問題文を1件にまとめています。表の欠落箇所は売上高÷販売単価で販売数量を補完しています。DFD・図表は省略または要旨のみです。25問制の場合は第25問は未収録です。

【学習のすすめ】解答解説書で正答を確認し、出題当時の用語・制度に留意してください（IT用語や試験範囲は改定されています）。
...
async function main() {
  ...
  const p1 = fs.readFileSync(path.join(__dirname, 'data', 'sme-h19-1ji-mis-part1.txt'), 'utf8');
  const p2 = fs.readFileSync(path.join(__dirname, 'data', 'sme-h19-1ji-mis-part2.txt'), 'utf8');
  const questionText =
    p1 +
    '\n' +
    p2 +
  ...
  const row: Record<string, unknown> = {
    ...
    choices: [
      {
        id: 'a',
        text: '【1次・選択式】上記は第1～24問の問題文です（第25問がある年度は公式問題集で補完）。正答は解答解説書で確認してください。',
      },
      {
        id: 'b',
