/**
 * 中小企業診断士 平成20年度 第1次試験「中小企業経営・中小企業政策」（1日目）全29問を
 * certification_questions に1レコードで追加（または更新）
 *
 * 実行（career-platform-nextjs で）:
 *   npx ts-node --transpile-only scripts/add-sme-h20-1ji-chisho-policy-question.ts
 */
import fs from 'fs';
...
const QUESTION_ID = 'sme-h20-1ji-chisho-policy';
...
const EXPLANATION = `【本問について】第1次試験・中小企業経営・中小企業政策（選択式・全29問）の問題文を1件にまとめています。OCRで欠けた数値・資本金区分・補助率・年数は「要確認」としています。正答と現行制度は解答解説書・協会資料で確認してください。

【学習のすすめ】統計・施策は試験年度時点の内容に留意し、制度改正後の差分を整理するとよいです。
...
async function main() {
  ...
  const p1 = fs.readFileSync(path.join(__dirname, 'data', 'sme-h20-1ji-chisho-policy-part1.txt'), 'utf8');
  const p2 = fs.readFileSync(path.join(__dirname, 'data', 'sme-h20-1ji-chisho-policy-part2.txt'), 'utf8');
  const questionText =
    p1 +
    '\n' +
    p2 +
  ...
  const row: Record<string, unknown> = {
    ...
    choices: [
      {
        id: 'a',
        text: '【1次・選択式】上記は全29問の問題文です。図表・数値の欠落は公式問題集で補完し、正答は解答解説書で確認してください。',
      },
      {
        id: 'b',
