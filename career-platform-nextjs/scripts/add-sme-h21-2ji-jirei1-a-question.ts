/**
 * 中小企業診断士 平成21年度 第2次試験 事例Ⅰ（A社・菓子メーカー）を certification_questions に追加（または更新）
 *
 * 実行（career-platform-nextjs で）:
 *   npx ts-node --transpile-only scripts/add-sme-h21-2ji-jirei1-a-question.ts
 */
import fs from 'fs';
...
const QUESTION_ID = 'sme-h21-2ji-jirei1-a';
...
【第1問】和菓子・地産地消・デパート網 vs 洋菓子・職人技・店舗併設工場など、事例の対比から強みの差を150字で。

【第2問】財務以外の逡巡理由（統合リスク、ブランド・組織文化、経営資源、後継・イメージ、戦略優先など）100字。

【第3問】PA比率・整理統合・配置転換・モラールの非対称（買収側の不安・不信）を人事構成と結びつけて100字。

【第4問】洋菓子ノウハウの移転、品質・開発、現場統合・士気、和洋融合など期待効果100字。

【第5問】短期売上（既存チャネル・通販・物産・キャンペーン・コラボ・コスト・撤退・選択と集中等）を150字。診断士の助言として具体性を。
...
async function main() {
  ...
  const txtPath = path.join(__dirname, 'data', 'sme-h21-2ji-jirei1-a-question.txt');
