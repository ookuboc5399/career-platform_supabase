/**
 * 中小企業診断士 平成20年度 第1次試験「経済学・経済政策」（1日目）第1～19問を
 * certification_questions に1レコードで追加（または更新）
 *
 * 実行（career-platform-nextjs で）:
 *   npx ts-node --transpile-only scripts/add-sme-h20-1ji-economics-question.ts
 */
import fs from 'fs';
...
const QUESTION_ID = 'sme-h20-1ji-economics';
...
const EXPLANATION = `【本問について】第1次試験・経済学・経済政策（選択式・全19問）の問題文を1件にまとめています。第6・8・9・10・18問など図表依存の設問は図を省略しているため、公式問題集の図と照合してください。

【学習のすすめ】ミクロ・マクロの正答は解答解説書で確認し、統計式・国際経済の分数式は教科書と突き合わせると理解が進みます。
...
async function main() {
  ...
  const p1 = fs.readFileSync(path.join(__dirname, 'data', 'sme-h20-1ji-economics-part1.txt'), 'utf8');
  const p2 = fs.readFileSync(path.join(__dirname, 'data', 'sme-h20-1ji-economics-part2.txt'), 'utf8');
  const questionText =
    p1 +
    '\n' +
    p2 +
  ...
  const row: Record<string, unknown> = {
    ...
    choices: [
      {
        id: 'a',
        text: '【1次・選択式】上記は第1～19問の問題文です。図・数式は公式問題集で補完し、正答は解答解説書で確認してください。',
      },
      {
        id: 'b',
