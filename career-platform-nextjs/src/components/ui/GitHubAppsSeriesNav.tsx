...
import {
  GITHUB_APPS_CHAPTER_SERIES,
  GITHUB_APPS_SERIES_TOTAL,
  getGitHubAppsSeriesIndex,
} from '@/lib/github-enterprise-github-apps-series';
...
export default function GitHubAppsSeriesNav({ languageId, currentChapterId, children }: Props) {
  const idx = getGitHubAppsSeriesIndex(currentChapterId);
  const prev = idx > 0 ? GITHUB_APPS_CHAPTER_SERIES[idx - 1] : null;
  const next =
    idx >= 0 && idx < GITHUB_APPS_CHAPTER_SERIES.length - 1
      ? GITHUB_APPS_CHAPTER_SERIES[idx + 1]
      : null;

  const current = idx >= 0 ? GITHUB_APPS_CHAPTER_SERIES[idx] : null;
  const base = `/programming/${languageId}/chapters`;

  return (
    <div className="mb-8 w-full max-w-4xl">
      <nav
        aria-label="GitHub Apps サブチャプター（推奨順）"
      ...
      >
          ...
          {current ? (
            <p className="text-sm font-medium text-gray-800 dark:text-gray-100">
              ...
              <span className="tabular-nums text-blue-600 dark:text-blue-400">
                {current.step}/{GITHUB_APPS_SERIES_TOTAL}
              </span>
              ：{current.navLabel}
            </p>
          ) : null}
        ...
        <p className="mb-4 text-sm text-gray-600 dark:text-gray-300">
          <strong className="font-semibold text-gray-800 dark:text-gray-100">1 → 2 → 3 → 4</strong>
          の順に進めると、概要 → App 作成 → 組織への導入 → Webhook 設定と自然につながります。どの項目からでも開けます。
        </p>

        <ol className="flex list-none flex-col gap-2 p-0 sm:flex-row sm:flex-wrap sm:items-center sm:gap-1">
          {GITHUB_APPS_CHAPTER_SERIES.map((s, i) => {
            ...
            return (
              <li key={s.id} className="flex items-center gap-1 sm:contents">
                ...
                {i < GITHUB_APPS_CHAPTER_SERIES.length - 1 ? (
                  <span
            ...
            <span className="flex flex-col items-start gap-0.5">
              ...
              <span>
                ...
                <span className="ml-1 tabular-nums text-xs text-gray-500 dark:text-gray-400">
                  （{prev.step}/{GITHUB_APPS_SERIES_TOTAL}）
                </span>
              </span>
            </span>
          </Link>
          ...
          >
            <span className="flex flex-col items-end gap-0.5">
              ...
              <span>
                ...
                <span className="ml-1 tabular-nums text-xs text-gray-500 dark:text-gray-400">
                  （{next.step}/{GITHUB_APPS_SERIES_TOTAL}）
                </span>
              </span>
            </span>
            <span aria-hidden>→</span>
