...
class RTAudioContent {
  ...
  private constructor(
    ...
    this.contentQueue = new SharedEndQueue(
      () => this.receiveContent(),
      (m: ServerMessageType | null) => m !== null && m.type === "error",
      (m: ServerMessageType | null) => m !== null && m.type === "response.content_part.done",
    );
  }
...
class RTFunctionCallItem implements AsyncIterable<string> {
  ...
  async waitForCompletion(): Promise<void> {
    ...
    for await (const _ of this.inner()) {
      // Consume all items to ensure completion
      continue;
    }
  }
}
...
class RTResponse implements AsyncIterable<RTOutputItem> {
  ...
  async cancel(): Promise<void> {
    ...
    for await (const _ of this) {
      // Consume all remaining items to ensure proper cleanup
      continue;
    }
