...
import { ProgrammingChapter, ProgrammingPracticeExercise } from '@/types/api';

interface AddPracticeExerciseModalProps {
  isOpen: boolean;
  onClose: () => void;
  languageId: string;
...
export default function AddPracticeExerciseModal({
  isOpen,
  onClose,
  languageId,
  chapters,
  existingExercises,
}: AddPracticeExerciseModalProps) {
  const [type, setType] = useState<'code' | 'multiple-choice'>('code');
  const [title, setTitle] = useState('');
  const [description, setDescription] = useState('');
  const [descriptionJapanese, setDescriptionJapanese] = useState(''); // 日本語の問題文
  const [languageMode, setLanguageMode] = useState<'japanese' | 'english'>('japanese'); // 言語切り替え
  ...
  const duplicateCheck = useMemo(() => {
    ...
    const duplicate = existingExercises.find(
      (exercise) =>
        (exercise.description?.trim() === trimmedDescription || 
         exercise.descriptionJapanese?.trim() === trimmedDescription) &&
        exercise.languageId === languageId
    );

    return duplicate || null;
  }, [description, descriptionJapanese, languageMode, existingExercises, languageId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/programming/practice-exercises', {
        method: 'POST',
        headers: {
      ...
      console.error('Error creating exercise:', error instanceof Error ? error.message : error);
      alert('問題の作成に失敗しました');
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-white">
        <DialogHeader>
          <DialogTitle>新規試験対策問題追加</DialogTitle>
        </DialogHeader>
