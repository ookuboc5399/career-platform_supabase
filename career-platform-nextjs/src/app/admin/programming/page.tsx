...
export default function ProgrammingAdminPage() {
  ...
  const aiItems = languages.filter((lang) => lang.type === 'ai');
  const uiUxItems = languages.filter((lang) => lang.type === 'ui-ux');
            ...
            {aiItems.length === 0 ? (
              ...
              aiItems.map((lang) => (
                <div
                  ...
                  className="bg-white rounded-lg shadow-lg hover:shadow-xl transition-shadow p-6 relative"
                >
                  <Link href={`/admin/programming/${lang.id}/chapters`} className="block">
                    <h3 className="text-lg font-semibold mb-2">{lang.title}</h3>
                    <p className="text-gray-600 mb-4">{lang.description}</p>
                    <div className="text-blue-600 font-medium">チャプター管理 →</div>
                  </Link>
                  <div className="absolute top-4 right-4 flex gap-2">
                    <button
                      type="button"
